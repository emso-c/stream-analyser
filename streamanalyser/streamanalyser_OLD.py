...
from .utils.structures import Message, Highlight, Author, Intensity
...
class StreamAnalyser:
    def __init__(
        ...
        utils.delete_old_files(
            os.path.join(self.cache_path, "src\\logs"), self.config["log-duration-days"]
        )
        ...
        try:
            self.metadata = yaml.load(
                open(f"{self.config['path-to']['metadata']}\\{stream_id}.yaml", "r"),
                Loader=yaml.Loader,
            )
        ...
        self.keyword_limit = (
            keyword_limit if keyword_limit else self.config["keyword-limit"]
        )
    ...
    def collect_stream_info(self) -> dict:
        ...
        params = {
            "format": "json",
            "url": "https://www.youtube.com/watch?v=%s" % self.stream_id,
        }
    ...
    def cache_messages(self):
        ...
        utils.delete_file(
            f"{self.config['path-to']['cache']}\\{self.stream_id}.json.gz"
        )
        raw_messages = []
        try:
            for counter, raw_message in enumerate(
                ChatDownloader().get_chat(self._youtube_url, start_time=0), start=1
            ):
                if self.verbose:
                    print(
                        f"Writing messages to {self.stream_id}.json...{str(utils.percentage(counter, self.limit))+'%' if self.limit else counter}",
                        end="\r",
                    )
                try:
                    raw_messages.append(
        ...
        try:
            with open(
                f"{self.config['path-to']['cache']}/{self.stream_id}.json",
                "w",
                encoding="utf-8",
            ) as f:
            ...
            self.logger.critical(
                f"Could not cache messages: {e.__class__.__name__}:{e}"
            )
            raise RuntimeError(f"Could not cache messages: {e.__class__.__name__}:{e}")

        try:
            with open(
                f"{self.config['path-to']['metadata']}\\{self.stream_id}.yaml", "w+"
            ) as file:
                yaml.dump(
                    {"is-complete": False if self.limit else True},
                    file,
                    Dumper=yaml.Dumper,
                )
        ...
        self.metadata = yaml.load(
            open(f"{self.config['path-to']['metadata']}\\{self.stream_id}.yaml", "r"),
            Loader=yaml.Loader,
        )
        ...
        try:
            ...
            with open(
                ...
                for count, raw_message in enumerate(raw_messages):
                    ...
                    if self.verbose:
                        print(
                            f"Reading messages from {self.stream_id}.json...{utils.percentage(count, self.limit if self.limit else len(raw_messages))}%",
                            end="\r",
                        )
                    messages.append(
                        Message(
    ...
    def get_messages(self) -> get_messages_from_json:
        ...
        if self.clear_cache or not os.path.isfile(
            f"{self.config['path-to']['cache']}/{self.stream_id}.json.gz"
        ):
            if self.verbose:
                print(f"creating {self.stream_id}.json")
            self.logger.debug(
                f"Clearing cache. Reason: {'forced' if self.clear_cache else 'couldnt find cache file'}"
            )
            self.cache_messages()
        else:
            if self.verbose:
            ...
            self.metadata = yaml.load(
                open(
                    f"{self.config['path-to']['metadata']}\\{self.stream_id}.yaml", "r"
                ),
                Loader=yaml.Loader,
            )
            with open(
                ...
                if self.limit and len(message_data) < self.limit:
                    if self.verbose:
                        print(
                            f"Missing messages detected in {self.stream_id}.json ({self.limit-len(message_data)} messages)"
                        )
                    self.logger.warning("Missing messages detected")
                    for counter, raw_message in enumerate(
                        ChatDownloader().get_chat(
                            self._youtube_url, start_time=latest_message_time
                        ),
                        start=len(message_data),
                    ):
                        if counter == self.limit:
                            break
                        if self.verbose:
                            print(
                                f"Writing messages to {self.stream_id}.json ({counter} messages)...",
                                end="\r",
                            )
                    ...
                    if self.verbose:
                        print(
                            f"Writing messages to {self.stream_id}.json ({len(message_data)} messages)... done"
                        )
                    self.logger.info(f"Written {self.limit-len(message_data)} messages")

                elif not self.limit and (
                    not "is-complete" in self.metadata.keys()
                    or not self.metadata["is-complete"]
                ):
                    ...
                    if self.verbose:
                        print(
                            "Message cache has not been fully filled before. Fetching all the messages."
                        )
                    self.logger.warning(
                        "Message cache has not been fully filled before. Fetching all the messages."
                    )
    ...
    def calculate_moving_average(self, table) -> dict:
        ...
        for time, value in table.items():
            if self.verbose:
                print(
                    f"Calculating moving average...{utils.percentage(time, len(table))}%",
                    end="\r",
                )
    ...
    def create_highlight_annotation(self) -> list[int]:
        ...
        self.logger.debug(
            f"Total decreasing duration: {highlight_annotation.count(-1)}"
        )
        ...
        for current_time in range(len(self.highlight_annotation)):
            ...
            if start_time and self.highlight_annotation[current_time] != 1:
                ...
                if duration < self.config["min-hl-duration"]:
                    self.logger.debug(
                        f"Highlight @{start_time} not added, duration was {duration}"
                    )
                    start_time = 0
                    continue
                ...
                if delta < 0:
                    self.logger.debug(
                        f"Highlight @{start_time} not added, delta was {delta}"
                    )
                    start_time = 0
                    continue
                ...
                for keyword in highlight.keywords:
                    for context in context_list:
                        for trigger in context["triggers"]:
                            if (
                                trigger["is_exact"] and trigger["phrase"] == keyword
                            ) or (
    ...
    def draw_graph(self) -> plt:
        ...
        # TODO background img
        img = mpimg.imread(
            f"{self.config['path-to']['thumbnails']}\\{self.stream_id}.jpg"
        )

        fig, ax = plt.subplots(2, constrained_layout=True)

        if os.path.exists(
            f"{self.config['path-to']['fonts']}\\NotoSansCJKjp-regular.otf"
        ):
            fprop = fm.FontProperties(
                fname=f"{self.config['path-to']['fonts']}\\NotoSansCJKjp-regular.otf"
            )
        else:
            fprop = None
...
and put the 'NotoSansCJKjp-regular.otf' file into {self.config['path-to']['fonts']}"
    ...
    def export_data(self, path=None, folder_name=None):
        ...
        copyfile(
            src=rf"{self.config['path-to']['cache']}/{self.stream_id}.json",
            dst=rf"{target_path}/messages.json",
        )
        utils.compress_file(f"{self.config['path-to']['cache']}\\{self.stream_id}.json")
        self.logger.info("Exported messages")

        copyfile(
            src=rf"{self.config['path-to']['metadata']}/{self.stream_id}.yaml",
            dst=rf"{target_path}/metadata.yaml",
        )
        self.logger.info("Exported metadata")

        copyfile(
            src=rf"{self.config['path-to']['thumbnails']}/{self.stream_id}.jpg",
            dst=rf"{target_path}/thumbnail.jpg",
        )
    ...
    def print_summary(self, top=None, intensity_filters=[]) -> list[Highlight]:
        ...
        for _count, highlight in enumerate(highlights):
            if not highlight.intensity in intensity_filters:
                print(
                    f"{timedelta(seconds=int(highlight.time))}: {highlight.intensity.colored_level}"
                )
    ...
    def print_highlights(
        self, top=None, include=[], not_include=[], intensity_filters=[]
    ) -> list[Highlight]:
    ...
    def print_urls(self, top=None, intensity_filters=[]) -> list[Highlight]:
        ...
        for _count, hl in enumerate(highlights):
            if not hl.intensity.level in intensity_filters:
                print(
                    hl.intensity.color
                    + str(timedelta(seconds=int(hl.time)))
                    + Style.RESET_ALL,
                    "->",
                    hl.url,
                )
    ...
    def most_used_word(self, exclude=[], normalize=True) -> Tuple[str, int]:
        ...
        self.logger.debug(
            f"Most used word: {Counter(_words).most_common(1+_idx)[_idx]}"
        )
        return Counter(_words).most_common(1 + _idx)[_idx]
    ...
    def least_recently_used_id(self) -> str:
        ...
        self.logger.debug(
            f"Least recently used id: {least_recently_used_id} ({last_access_time})"
        )
    ...
    def set_thumbnail_url(self, res_lvl=2) -> str:
        ...
        self.thumbnail_url = (
            f"https://i.ytimg.com/vi/{self.stream_id}/{res_lvls[res_lvl]}.jpg"
        )
    ...
    def download_thumbnail(self):
        ...
        try:
            ...
            with open(
                f"{self.config['path-to']['thumbnails']}\\{self.stream_id}.jpg", "wb"
            ) as f:
    ...
    def cached_ids(self) -> list[str]:
        ...
        return [
            fname.split(os.extsep)[0]
            for fname in os.listdir(self.config["path-to"]["cache"])
        ]
    ...
    def check_file_integrity(self) -> list[str]:
        ...
        for cache_folder in cache_folders:
            ...
            self.logger.debug(
                f"Checking difference between {largest_folder} and {cache_folder}"
            )
            diffs = list(
                set(utils.filenames(largest_folder))
                - set(utils.filenames(cache_folder))
            )
            for diff in diffs:
