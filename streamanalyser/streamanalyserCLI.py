...
def parseargs():
    ...
    parser.add_argument("-if", "--intensity-filters", nargs='+', default=[], 
        help="highlight intensity levels to filter out")
    parser.add_argument("-kf", "--keyword-filters", nargs='+', default=[], 
        help="keywords to filter out")
    parser.add_argument("-inc", "--include-context", nargs='+', default=[], 
        help="keywords to filter out")
    parser.add_argument("-exc", "--exclude-context", nargs='+', default=[], 
        help="keywords to filter out")
    ...
    parser.add_argument("--thumb-res-lvl", default=2, type=int,
        help="thumbnail resolution level (0-3) with 0 being lowest and 3 being highest")
    parser.add_argument("-dl", "--disable-logs", action="store_true", 
        help="actions done withing the current session will not be logged")
    parser.add_argument("-ld", "--log-duration", default=15, type=int,
        help="how old a log file should be to get deleted (in days)")
    parser.add_argument("-r", "--reset", action="store_true", 
        help="clear cache before analysing")
    parser.add_argument("-nc", "--not-cache", action="store_true", 
        help="clear cache after analysing ")
    parser.add_argument("-md", "--min-duration", default=15, type=int,
        help="minimum highlight duration in seconds")
    parser.add_argument("-oef", "--open-export-folder", action="store_true", 
        help="open the export folder in file explorer after exporting")
    parser.add_argument("-wc", "--wordcloud", action="store_true", 
        help="show word cloud")
    parser.add_argument("-wcs", "--wordcloud-scale", default=3, type=int,
        help="scale of the wordcloud")
...
def main():
    ...
    with analyser:
        ...
        top_highlights = analyser.get_highlights(
            top=args.top, output_mode=mode, include=args.include_context,
            exclude=args.exclude_context, intensity_filters=args.intensity_filters
        )
