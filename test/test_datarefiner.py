...
class test(unittest.TestCase):
    def test_refine_raw_messages(self):
        ...
        self.assertEqual(result_messages, expected_messages)

        # w/ limit
        result_messages = self.refiner.refine_raw_messages(
            sample_raw_messages, msglimit=3
        )
        self.assertEqual(result_messages, expected_messages[:3])
    ...
    def test_get_authors(self):
        ...
        result_authors = self.refiner.get_authors()
        expected_authors = [Author(id=i, name="name" + str(i)) for i in range(6)]
        self.assertEqual(result_authors, expected_authors)
