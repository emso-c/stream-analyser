...
class TestStreamAnalyser(unittest.TestCase):
   ...
   def test_analyse_data(self):
      with sa.StreamAnalyser('um196SMIoR8', 1, True) as analyser:
         ...
         analyser.messages = analyser.refiner.refine_raw_messages(raw_messages)
         analyser.set_context_path(
            "C:\\Users\\emirs\\Desktop\\stream-analyser\\streamanalyser\\data\\context.json"
         )
         analyser.analyse_data()

         self.assertEqual(
            analyser.highlights[0].colorless_str,
            "[0:00:01] greeting: こんやっぴートワ様いえーい, こん, こん…, kon (24 messages, high intensity, 1.042 diff, 19s duration)"
         )
